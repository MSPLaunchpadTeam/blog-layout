...
(function () {
  ...
  function calloutRuns(list) {
    ...
    var flush = function () {
      ...
      // A paragraph leading into a list or another raised block is part of that explanation, not an aside.
      var next = last && last.nextElementSibling;
      var connected = next && (/^(UL|OL)$/.test(next.tagName) || /mspl-ig|mspl-lede|mspl-hl|mspl-callout/.test(next.className || ''));
      if (run.length >= RUN_MIN && last && !connected && !/:\s*$/.test(last.textContent) && last.className.indexOf('mspl-lede') === -1) { addClass(last, 'mspl-callout'); made++; }
      run = [];
    };
    for (var i = 0; i < list.length; i++) {
